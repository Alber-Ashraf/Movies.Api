    ...
    public class AuthTokenProvider
    {
        ...
        public async Task<string> GetTokenAsync()
        {
            if (!string.IsNullOrEmpty(_cachedToken))
            {
                ...
                var expiration = jwt.Claims.Single(claim => claim.Type == "exp").Value;
                var expirationDateTime = UnixTimeStampToDateTime(int.Parse(expiration));
            ...
            var response = await _httpClient.PostAsJsonAsync("https://localhost:7288/token", new
            {
